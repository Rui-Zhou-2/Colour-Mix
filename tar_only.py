...
def main(args: argparse.Namespace):
    ...
    # create model
    model = models.__dict__[args.arch](num_classes=train_dataset.num_classes).to(device)
    ...
    decode = train_dataset.decode_target
    ...
    if args.phase == 'test':
        confmat = validate(val_loader, model, interp_val, criterion, visualize, args)
        print(confmat)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_iter, model, interp_train, criterion, optimizer,
              lr_scheduler, epoch, visualize if args.debug else None, args)

        # evaluate on validation set
        confmat = validate(val_loader, model, interp_val, criterion, visualize if args.debug else None, args)
        print(confmat.format(train_dataset.classes))
        acc_global, acc, iu = confmat.compute()

        # calculate the mean iou over partial classes
        indexes = [train_dataset.classes.index(name) for name
                   in train_dataset.evaluate_classes]
        iu = iu[indexes]
        mean_iou = iu.mean()
...
def train(train_iter: ForeverDataIterator, model, interp, criterion, optimizer: SGD,
          lr_scheduler: LambdaLR, epoch: int, visualize, args: argparse.Namespace):
    batch_time = AverageMeter('Time', ':4.2f')
    data_time = AverageMeter('Data', ':3.1f')
    losses = AverageMeter('Loss1 (s)', ':3.2f')
...
if __name__ == '__main__':
    ...
    # dataset parameters
    parser.add_argument('data_root', help='root path of the dataset')
    parser.add_argument('--data', help='domain(s)')
    ...
    parser.add_argument("--resume", type=str, default='./rgb/tar_cj_0.2/ckp/best.pth',
