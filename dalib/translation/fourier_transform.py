...
class FourierTransform(nn.Module):
    ...
    def __init__(self, image_list: List[str], amplitude_dir: str,
    ...
    def build_amplitude(image_list, amplitude_dir):
        ...
        for image_name in tqdm.tqdm(image_list):
            image_name_without_path = os.path.basename(image_name)  # Remove directory path
            image_name_without_extension = os.path.splitext(image_name_without_path)[0]  # Remove file extension
            ...
            amp = np.abs(fft)
            np.save(os.path.join(amplitude_dir, "{}.npy".format(image_name_without_extension)), amp)


    def forward(self, image):
        # randomly sample a target image and load its amplitude component
        image_index = random.randint(0, self.length-1)
        image_name = os.path.basename(self.image_list[image_index])  # Remove directory path
        image_name_without_extension = os.path.splitext(image_name)[0]  # Remove file extension

        amp_trg = np.load(os.path.join(self.amplitude_dir, "{}.npy".format(image_name_without_extension)))
