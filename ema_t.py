...
from self_training.mean_teacher import update_bn, EMATeacher2
...
def main(args: argparse.Namespace):
    ...
    # create model
    model = models.__dict__[args.arch](num_classes=2).to(device)#train_source_dataset.num_classes
    ...
    if args.phase == 'test':
        confmat = validate(val_target_loader, t_model, interp_val, criterion, visualize, args)
        print(confmat)
        return
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        #     mixed_label[j] = label_t_c.clone()
        #     mixed_label[j][mask] = label_s_c[mask]

        #     mask = mask.unsqueeze(0).expand(x_t[j].size(0), -1, -1)  # Expand the mask dimensions to match the number of channels

        #     mixed_x_t[j] = x_t[j].clone()
        #     mixed_x_t[j][mask] = x_s[j][mask]    

        # mixed_x_t = torch.stack(mixed_x_t)
        ...
        # confmat_m.update(mixed_label.flatten(), pred_mix.argmax(1).flatten())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', default=18, type=int,
                        help='seed for initializing training. 3407')
